    ...
    def __init__(self):
        def gotinfo(pointless=None):
            ...
            try:
                ...
                if c == 1:
                    n = 1
                    TOKEN = s.token; NAME = s.name; ADDRESS = s.address; MOBILE_NO = s.mobile_no; PEOPLE = s.no_of_people; ROOM_TYPE = s.room_type; DAYS = s.no_of_days; TOTAL_BILL = s.price
                    self.Text1.insert(INSERT, "TOKEN NO. -\t\t{}".format(TOKEN))
                    self.Text1.insert(INSERT, "\nNAME -\t\t{}".format(NAME))
                    self.Text1.insert(INSERT, "\nADDRESS -\t\t{}".format(ADDRESS))
                    self.Text1.insert(INSERT, "\nMOBILE NO. -\t\t{}".format(MOBILE_NO))
                    self.Text1.insert(INSERT, "\nNO. OF PEOPLE -\t\t{}".format(PEOPLE))
                    self.Text1.insert(INSERT, "\nROOM TYPE -\t\t{}".format(ROOM_TYPE))
                    self.Text1.insert(INSERT, "\nNO. OF DAYS -\t\t{}".format(DAYS))
                    self.Text1.insert(INSERT, "\nTOTAL BILL -\t\tRs.{}\n\n".format(TOTAL_BILL))
            except EOFError:
                self.Text1.insert(INSERT, "NO GUEST IN THE ROOM NO. {}\n\n".format(self.gettininfo))
                n = 0
                pass
                f2.close()
        ...
        font12 = "-family {Helvetica Neue Light} -size 15 -weight normal -slant"  \
            " roman -underline 0 -overstrike 0"
        ...
        self.Frame1.configure(relief=GROOVE)
        self.Frame1.configure(background="#556B2F")
        ...
        self.Text1.configure(background="#BDB76B")
